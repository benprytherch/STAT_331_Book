...
• The mean of a variable 𝑋 is denoted 𝑋ത.
...
• Try creating a skewed data set in jamovi, then analyzing it by selecting for mean and median in the Statistics drop down menu in Descriptives, found under Exploration in the Analysis tab • To create a new dataset, enter data into the blank data table jamovi creates by default
...
• The variance of a distribution or data set is denoted 𝑠2:
...
s = • Think of this as the "standard" amount by which values deviate from their mean.
...
• When two variables move in the opposite direction (i.e. when one gets bigger, the other gets smaller), 𝑟 is negative.

• When they move in the same direction, 𝑟 is positive.

• 𝑟 = 0 means no correlation. 𝑟 = 1 means perfect positive correlation. 𝑟 = −1 means perfect negative correlation.
...
• Example: a sample mean can be used to estimate a population mean. i.e. 𝑋ത estimates 𝜇.
